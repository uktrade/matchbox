"""First creation at current state at implementation of alembic.

Revision ID: 56e708841fc1
Revises:
Create Date: 2025-04-04 17:18:29.245098
...
def upgrade() -> None:
    ...
    sa.CheckConstraint("type IN ('model', 'dataset', 'human')", name='resolution_type_constraints'),
    ...
    sa.ForeignKeyConstraint(['child'], ['mb.clusters.cluster_id'], ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['parent'], ['mb.clusters.cluster_id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('parent', 'child'),
    schema='mb'
    )
    op.create_index('ix_contains_child_parent', 'contains', ['child', 'parent'], unique=False, schema='mb')
    op.create_index('ix_contains_parent_child', 'contains', ['parent', 'child'], unique=False, schema='mb')
    op.create_table('probabilities',
    ...
    sa.ForeignKeyConstraint(['cluster'], ['mb.clusters.cluster_id'], ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['resolution'], ['mb.resolutions.resolution_id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('resolution', 'cluster'),
    schema='mb'
    )
    op.create_table('resolution_from',
    sa.Column('parent', sa.BIGINT(), nullable=False),
    sa.Column('child', sa.BIGINT(), nullable=False),
    ...
    sa.ForeignKeyConstraint(['child'], ['mb.resolutions.resolution_id'], ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['parent'], ['mb.resolutions.resolution_id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('parent', 'child'),
    schema='mb'
    )
    op.create_table('sources',
    sa.Column('source_id', sa.BIGINT(), autoincrement=True, nullable=False),
    ...
    sa.ForeignKeyConstraint(['resolution_id'], ['mb.resolutions.resolution_id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('source_id'),
    sa.UniqueConstraint('full_name', 'warehouse_hash', name='unique_source_address'),
    schema='mb'
    )
    op.create_table('cluster_source_pks',
    ...
    sa.ForeignKeyConstraint(['cluster_id'], ['mb.clusters.cluster_id'], ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['source_id'], ['mb.sources.source_id'], ondelete='CASCADE'),
    sa.PrimaryKeyConstraint('pk_id'),
    sa.UniqueConstraint('pk_id', 'source_id', name='unique_pk_source'),
    schema='mb'
    )
    op.create_index('ix_cluster_source_pks_cluster_id', 'cluster_source_pks', ['cluster_id'], unique=False, schema='mb')
    op.create_index('ix_cluster_source_pks_source_pk', 'cluster_source_pks', ['source_pk'], unique=False, schema='mb')
    op.create_table('source_columns',
    sa.Column('column_id', sa.BIGINT(), autoincrement=True, nullable=False),
    sa.Column('source_id', sa.BIGINT(), nullable=False),
    ...
    op.create_index('ix_source_columns_source_id', 'source_columns', ['source_id'], unique=False, schema='mb')
...
def downgrade() -> None:
    """Downgrade schema."""
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_index('ix_source_columns_source_id', table_name='source_columns', schema='mb')
    op.drop_table('source_columns', schema='mb')
    op.drop_index('ix_cluster_source_pks_source_pk', table_name='cluster_source_pks', schema='mb')
    op.drop_index('ix_cluster_source_pks_cluster_id', table_name='cluster_source_pks', schema='mb')
